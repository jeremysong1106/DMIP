...
public class ExercisePB {
	...
	public Grid1D rampFiltering(Grid1D projection, RampFilterType filterType){
		...
		if (!filterShownOnce)
		    ...
		    ramp.transformForward();// <your code> // TODO: Transform the filter into frequency domain (look for an appropriate method of Grid1DComplex)
		...
		projectionF.transformForward();// <your code> // ... into the frequency domain (hint: similar to the last TODO)
		
		if (projectionF != null) {
			for(int p = 0; p < projectionF.getSize()[0]; p++){
				projectionF.multiplyAtIndex(p, ramp.getRealAtIndex(p),ramp.getImagAtIndex(p));// <your code> // TODO: Multiply the transformed sinogram with the ramp filter (complex multiplication) 
			}
		}

		projectionF.transformInverse();// <your code> // TODO: transform back to get the filtered sinogram (i.e. invert the Fourier transform)
	...
	public void ramlak(Grid1DComplex filterGrid, int paddedSize, double deltaS) {
		...
		filterGrid.setAtIndex(0, (float)(1/(4*deltaS*deltaS)));// <your code>  // TODO: set correct value in filterGrid for zero frequency
		
		for (int i = 1; i < paddedSize/2; ++i) { // the "positive wing" of the filter 
			
			if (i%2==1) {// TODO: condition -> only odd indices are nonzero
				filterGrid.setAtIndex(i, constantFactor/i/i);// <your code> // TODO: use setAtIndex and the constant "constantFactor"
			}
		}
		...
		for (int i = paddedSize / 2; i < paddedSize; ++i) { 
			...
			if (i%2==1) {// TODO: condition -> only odd indices are nonzero
				filterGrid.setAtIndex(i, constantFactor/tmp/tmp);// <your code> // TODO: use setAtIndex and the constant "constantFactor"
			}
		}
	}
	...
	public void sheppLogan(Grid1DComplex filterGrid, int paddedSize, double deltaS) {
		...
		filterGrid.setAtIndex(0, (float)(2/(Math.PI*Math.PI*deltaS*deltaS)));// <your code> // TODO: set correct value in filterGrid for zero frequency
		
		for (int i = 1; i < paddedSize/2; ++i){ // the "positive wing" of the filter
			filterGrid.setAtIndex(i, constantFactor/(4*i*i-1));// <your code> // TODO: use setAtIndex and the constant "constantFactor"
		}
		...
		for (int i = paddedSize / 2; i < paddedSize; ++i) {
			...
			filterGrid.setAtIndex(i, constantFactor/(4*tmp*tmp-1));// <your code> // TODO: use setAtIndex and the constant "constantFactor"
		}
	}

...
public class ExerciseFB {
	...
	public Grid2D applyCosineWeights(Grid2D sino) {
		...
		//System.out.println(detectorPixels);
	...
	public Grid2D backproject(Grid2D sino, int[] recoSize, float[] spacing) {
		...
		for(int p = 0; p < numProjs; p++) {
			...
			float wx, wy;
			//System.out.println(recoSize[0]);
			//System.out.println(spacing[0]);
			...
			for(int x = 0; x < recoSize[0]; x++) {
				...
				for(int y = 0; y < recoSize[1]; y++) {
					...
					final StraightLine projectionLine = new StraightLine(source, reconstructionPointWorld); //TODO
	...
	public ExerciseFB() {
		...
		System.out.print(detectorPixels);
		initParameters();

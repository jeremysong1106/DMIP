	...
	public void getLatticePoints (Grid2D distortedImage){
		...
		for(int i = 0; i < nx; i++){
			for(int j = 0; j < ny; j++){
				...
				Xu.setElementValue(i, j, i*a);//TODO: fill matrix Xu
				Yu.setElementValue(i, j, j*b);//TODO: fill matrix Yu
				Xd.setElementValue(i, j, i*fx);//TODO: fill matrix Xd
				Yd.setElementValue(i, j, j*fy);//TODO: fill matrix Yd
			}
		}
	}
			...
			for(int j = 0; j < imageHeight; j++){
				...
				for(int k = 0; k <= degree; k++){
					for(int l = 0; l <= degree - k; l++){
						...
						val1 = (float) (Math.pow(XdVector.getElement(cc), k)*Math.pow(YdVector.getElement(cc), l)*u_vec.getElement(cc));// TODO
						val2 = (float) (Math.pow(XdVector.getElement(cc), k)*Math.pow(YdVector.getElement(cc), l)*v_vec.getElement(cc));// TODO
						
						xDist.addAtIndex(i, j, val1);// TODO: fill xDist
						yDist.addAtIndex(i, j, val2);// TODO: fill yDist
						
						cc++;
					}
				}
				//xDist.subAtIndex(i, j, (float) Xd.getElement(i, j));
				//yDist.subAtIndex(i, j, (float) Yd.getElement(i, j));
			}
		}
